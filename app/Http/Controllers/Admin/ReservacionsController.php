...
class ReservacionsController extends Controller
{
    ...
    public function store(StoreReservacionsRequest $request)
    {
        ...
        /*while($tNow <= $tEnd){
            $x = date("H:i",$tNow);
            $val_reservation= DB::connection('odbc')->selectOne("SELECT id FROM reservaciones WHERE id_seccion = ".$request->sala_de_juntas." AND fecha_inicio = ".$f_inicio." AND hora_inicio=".$tNow." AND ");    
